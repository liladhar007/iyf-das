...
const FacilitatorDetails = () => {
  ...
  const [data, setData] = useState<Student[]>([]); // Always initialize as an empty array
  const [selectedRow, setSelectedRow] = useState<Student | null>(null);
  const [open, setOpen] = useState(false);
  const [rowSelection, setRowSelection] = useState<Record<string, boolean>>({});
  const [isLoading, setIsLoading] = useState(false);
  ...
  const fetchGetStudentGroupWise = async (group_name: string) => {
    try {
      setIsLoading(true);
      const response = await getFrontlinerdetailReport(facilitatorId, group_name);
      const users = response?.users || []; // Safely extract users or fallback to an empty array
      setData(users);
    } catch (err) {
      console.log('Failed to fetch students by group');
      setData([]); // If there's an error, ensure data is reset to an empty array
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <div className="mt-10">
        ...
        <div className="mb-5 mt-0 rounded-md bg-white p-5 shadow-2xl">
          {/* Only render MaterialReactTable when data is not empty */}
          {data.length > 0 ? (
            <MaterialReactTable
              columns={columns}
              data={data}
